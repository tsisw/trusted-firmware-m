...
{
    struct tsi_sacfg_block_t* p_sacfg =
                         (struct tsi_sacfg_block_t*)dev->cfg->sacfg_base;
    struct tsi_nsacfg_block_t* p_nsacfg =
                       (struct tsi_nsacfg_block_t*)dev->cfg->nsacfg_base;
    ...
    switch(dev->cfg->ppc_name) {
        ...
        case PPC_TSI_PERIPH_EXP3:
            ...
            break;
        case TSI_PPC_MAX_NUM:
        default:
            return PPC_TSI_ERR_INVALID_PARAM;
        }

    dev->data->is_initialized = true;

    return PPC_TSI_ERR_NONE;
}
                            ...
                            enum ppc_tsi_priv_attr_t priv_attr)
{
    if(dev->data->is_initialized != true) {
        return PPC_TSI_ERR_NOT_INIT;
    }

    if(sec_attr == PPC_TSI_SECURE_ACCESS) {
#if (defined (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U))
        /* Uses secure unprivileged access address (SACFG) to set privilege
         * attribute
         */
        if(priv_attr == PPC_TSI_PRIV_ONLY_ACCESS) {
        ...
        return PPC_TSI_ERR_NOT_PERMITTED;
#endif
    } else {
