...
{ \
    enum ppc_tsi_error_t ret; \
 \
    ret = ppc_tsi_config_privilege(&PPC_DEV, periph, \
                        (enum ppc_tsi_sec_attr_t)sec_attr, \
                        (enum ppc_tsi_priv_attr_t)priv_attr); \
 \
    if( ret != PPC_TSI_ERR_NONE) { \
...
{ \
    return ppc_tsi_is_periph_priv_only(&PPC_DEV, periph); \
} \
\
static int32_t \
...
{ \
    enum ppc_tsi_error_t ret; \
 \
    ret = ppc_tsi_config_security(&PPC_DEV, periph, \
                                (enum ppc_tsi_sec_attr_t)sec_attr); \
 \
    if( ret != PPC_TSI_ERR_NONE) { \
        return ARM_DRIVER_ERROR; \
...
{ \
    return ppc_tsi_is_periph_secure(&PPC_DEV, periph); \
} \
 \
...
{ \
    enum ppc_tsi_error_t ret; \
 \
    ret = ppc_tsi_irq_enable(&PPC_DEV); \
 \
    if( ret != PPC_TSI_ERR_NONE) { \
        return ARM_DRIVER_ERROR; \
    } \
...
{ \
    ppc_tsi_irq_disable(&PPC_DEV); \
} \
 \
static void PPC_NAME##_ClearInterrupt(void) \
{ \
    ppc_tsi_clear_irq(&PPC_DEV); \
} \
 \
static bool PPC_NAME##_InterruptState(void) \
{ \
    return ppc_tsi_irq_state(&PPC_DEV); \
} \
 \
/* PPC tsi PPC_NAME Driver CMSIS access structure */ \
ARM_DRIVER_PPC_TSI Driver_##PPC_NAME = { \
    .GetVersion        = PPC_TSI_GetVersion, \
